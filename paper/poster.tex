...
% Custom block style: like Default but with thinner border, smaller rounding
\defineblockstyle{Clean}{
    titlewidthscale=1, bodywidthscale=1, titlecenter,
    titleoffsetx=0pt, titleoffsety=0pt, bodyoffsetx=0pt, bodyoffsety=0pt,
    bodyverticalshift=0pt, roundedcorners=8, linewidth=0.08cm,
    titleinnersep=0.7cm, bodyinnersep=0.8cm
}{
...
\renewcommand{\TP@maketitle}{%
  ...
  \vbox{%
    \begin{minipage}[c]{0.3\linewidth}
      \raggedleft
      \includegraphics[height=5cm]{AUB_Main_Horizontal_RGB.pdf}
    \end{minipage}
    \hspace{0.05\linewidth}
    \begin{minipage}[c]{0.6\linewidth}
      \centering
      \color{titlefgcolor}
      {\bfseries \Huge \sc \shortstack{\textbf{How Stable Are Regression Conclusions Across Subpopulations?}\\[2mm] \large A Finite-Sample Empirical Study} \par}
      \vspace*{1em}
      {\huge [Authors] -- Department of Statistics -- July 2026 \par}
    \end{minipage}
  }%
}
\makeatother
  ...
  \begin{center}
    \begin{tikzpicture}[every node/.style={align=center, font=\small}]
      \node[draw=DarkBlue!30, fill=DarkBlue!5, rounded corners=4pt,
            inner sep=5pt, minimum width=0.22\linewidth] (A) {
            ``The coefficients differ\\across groups!''
          };
      \node[draw=DarkBlue!30, fill=DarkBlue!5, rounded corners=4pt,
            inner sep=5pt, minimum width=0.22\linewidth, below=1.2cm of A] (B) {
            ``Genuine heterogeneity.\\Model doesn't transport.''
          };
      \node[draw=AccentTeal!70, fill=AccentTeal!8, rounded corners=4pt,
            inner sep=5pt, minimum width=0.22\linewidth, below=1.2cm of B] (C) {
            \color{AccentTeal}``Wait --- different $X$\\ distributions cause\\most of the drift.''
          };
      \draw[->, DarkBlue!40, line width=1pt] (A.south) -- (B.north);
      \draw[->, DarkBlue!40, line width=1pt] (B.south) -- (C.north);
    \end{tikzpicture}
  \end{center}
...
\block{Real-Data: NHANES BPSysAve}{
  \textbf{NHANES 2017--2018} ($n=9{,}254$). Response: systolic BP.
  Subgroups: age decades. Focus predictor: BPSysAve.

  \begin{center}
    \includegraphics[width=0.7\linewidth]{../results/nhanes_age_decade/bpsys_coef_by_age_decade_ci.png}
  \end{center}
  ...
  \begin{tabular}{@{}lr@{}}
    ...
    SSR (BPSysAve)      & 0.57 \\
    Std.\ drift         & 32.07 \\
    Context-norm.\ drift & 7.49 \\
    \bottomrule
  \end{tabular}

  \textbf{Key finding:} Effect \textit{reverses sign} in older groups. Drift 32$\to$7.5 after context normalization, confirming genuine heterogeneity.
}
